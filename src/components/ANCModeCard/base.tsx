...
const ANCSliderButton = styled(Button, {
  shouldForwardProp: (prop) => prop !== 'position'
})<ANCSliderSwitcherProps>(({ position }) => ({
  display: 'flex',
  flex: 1,
...
export default function BaseANCModeCard() {
  ...
  const [sliderPosition, setSliderPosition] = useState<AllowedSliderPositions>(null);
  const [sliderIcon, setSliderIcon] = useState<string>(NormalIcon);
  const [submenuOpen, setSubmenuOpen] = useState<boolean>(false);
  const [ancModeSelected, setAncModeSelected] = useState<ANCModes>({ mode: 'AncOutdoorMode' });
  const [transModeSelected, setTransModeSelected] = useState<ANCModes>({
    mode: 'TransparencyFullyTransparentMode'
  });
  const [ancCustomValue, setAncCustomValue] = useState<number | number[] | null>(10);

  const ancButtons: Array<[string, ANCModes]> = [
  ...
  const transButtons: Array<[string, ANCModes]> = [
  ...
  useEffect(() => {
    ...
    if (currentANCMode.mode == 'NormalMode') {
      ...
      currentANCMode.mode == 'TransparencyFullyTransparentMode' ||
      currentANCMode.mode == 'TransparencyVocalMode'
    ) {
      ...
      <Grid sx={{ paddingLeft: 0, justifyContent: 'center' }}>
        ...
        <Grid item sx={{ paddingTop: '0px !important' }}>
          <Collapse in={submenuOpen}>
            <ButtonGrid
              buttonArray={sliderPosition == 'left' ? ancButtons : transButtons}
              setButtonSelected={
                sliderPosition == 'left' ? setAncModeSelected : setTransModeSelected
              }
              buttonSelected={sliderPosition == 'left' ? ancModeSelected : transModeSelected}>
...
function ButtonGrid({
  ...
  children: ReactNode;
  buttonArray: Array<[string, ANCModes]>;
  setButtonSelected: React.Dispatch<React.SetStateAction<ANCModes>>;
  buttonSelected: { mode: string };
}) {
  return (
    <Stack>
      <Grid
        ...
        {buttonArray.map(([title, mode]) => (
          <Grid item key={title}>
            <ANCModeButton
              variant="outlined"
              active={buttonSelected.mode == mode.mode}
              onClick={() => {
